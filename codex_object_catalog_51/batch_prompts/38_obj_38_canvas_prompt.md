# CODEX PROMPT — Холст на мольберте / Canvas on Easel
...
## Текущий объект
- `id`: `obj_38_canvas`
- `displayNameRu`: `Холст на мольберте`
- `displayNameEn`: `Canvas on Easel`
- `category`: `artObject`
- `family`: `flatMounted`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templateCanvas`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 80` и `maxTris = 160`.
...
- required parts: canvasPlane, frameThin
- required hooks: easel_canvas_mount
- required material slots: paper, paintedFrame
