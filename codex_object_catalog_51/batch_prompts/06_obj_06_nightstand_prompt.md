# CODEX PROMPT — Прикроватная тумбочка / Nightstand
...
- `displayNameRu`: `Прикроватная тумбочка`
- `displayNameEn`: `Nightstand`
- `category`: `furniture`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templateCabinetSmall`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 600` и `maxTris = 950`.
...
- required parts: body, top, drawerOrFrontPanel, handle
- required hooks: top_surface_center, front_face_center
- required material slots: woodMain, woodAccent, metalAccent
- deformation profile: rigid
- animation potential: drawer_pull_axis
- notes: Даже если ящик не анимируется сейчас, ось выдвижения нужна.
