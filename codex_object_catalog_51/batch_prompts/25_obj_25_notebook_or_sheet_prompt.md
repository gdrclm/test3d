# CODEX PROMPT — Блокнот/листок на столе / Notebook or Sheet on Desk
...
- `displayNameRu`: `Блокнот/листок на столе`
- `displayNameEn`: `Notebook or Sheet on Desk`
- `category`: `paperProp`
- `family`: `flatPaper`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templatePaperProp`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 50` и `maxTris = 120`.
...
- required parts: paperBody, lineOrCoverZone
- required hooks: desktop_attach_center
- required material slots: paper
- deformation profile: semiSoft
- animation potential: page_flip_optional
- notes: Очень лёгкий prop, без лишнего объёма.
