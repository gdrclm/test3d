# CODEX PROMPT — Ящик для игрушек / Toy Box
...
## Текущий объект
- `id`: `obj_28_toy_box`
- `displayNameRu`: `Ящик для игрушек`
- `displayNameEn`: `Toy Box`
- `category`: `furniture`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templateToyBox`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 700` и `maxTris = 1100`.
...
- required parts: body, lid, frontAccent
- required hooks: lid_hinge_back, top_surface_center
- required material slots: plasticMain, woodAccent, fabricAccent
- deformation profile: rigid
- animation potential: lid_open
- notes: Крупный детский контейнер с мягкими радиусами.
