# CODEX PROMPT — Кресло-мешок / Bean Bag
...
## Текущий объект
- `id`: `obj_36_bean_bag`
- `displayNameRu`: `Кресло-мешок`
- `displayNameEn`: `Bean Bag`
- `category`: `furniture`
- `family`: `softSeating`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templateBeanBag`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 650` и `maxTris = 1100`.
...
- required parts: mainSoftMass, topCompression, seamZone
- required hooks: floor_attach, seat_center
- required material slots: fabricMain, fabricAccent
- deformation profile: soft
- animation potential: compression_response, soft_secondary_motion
- notes: Высокоприоритетный объект для deformation hooks.
