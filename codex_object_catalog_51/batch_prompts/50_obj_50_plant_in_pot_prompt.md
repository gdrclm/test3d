# CODEX PROMPT — Растение в горшке / Plant in Pot
...
- `displayNameRu`: `Растение в горшке`
- `displayNameEn`: `Plant in Pot`
- `category`: `decor`
- `family`: `plantProp`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templatePottedPlant`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 400` и `maxTris = 800`.
...
- required parts: pot, leafClusterA, leafClusterB, leafClusterC
- required hooks: floor_attach, plant_secondary_sway
- required material slots: plantPot, plantLeaf
- deformation profile: mixed
- animation potential: plant_secondary_sway
- notes: Горшок жёсткий, листья полумягкие.
