# CODEX PROMPT — Потолочная лампа / Ceiling Lamp
...
- `displayNameRu`: `Потолочная лампа`
- `displayNameEn`: `Ceiling Lamp`
- `category`: `lighting`
...
1. Проанализируй виды и выдели:
   ...
   - uncertain areas
2. Проверь соответствие template `templateCeilingLamp`.
...
5. Зарегистрируй:
   ...
   - deform zones
   - animation hooks
6. Соблюди `targetTris = 380` и `maxTris = 700`.
...
- required parts: ceilingCap, hanger, lampBody, lightEmitter
- required hooks: ceiling_mount, light_emitter
- required material slots: metalAccent, glass, plasticMain
- deformation profile: rigid
- animation potential: gentle_sway_optional
- notes: Нужен отдельный слит light_emitter.
